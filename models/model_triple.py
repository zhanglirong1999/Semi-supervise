...
class RoBERTATripletModel(nn.Module):
    ...
    def forward(self, tokens):
        """
            tokens: input_ids: 
        """
        anchor = self.model(tokens['ids_anchor'],
                             attention_mask=tokens['mask_anchor'])
        positive = self.model(tokens['ids_positive'],
                             attention_mask=tokens['mask_positive'])
        negative = self.model(tokens['ids_negative'],
                             attention_mask=tokens['mask_negative'])   
        # print(anchor)
